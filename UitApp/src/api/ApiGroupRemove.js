...
_getGroupDatabase = (groupName) => {
  try {
    ...
    console.log("_getGroupDatabase ApiGroupRemove.js", error)
  }
};
_hashNode = (mes) => {
...
_SaveGroupDatabase3 = (tree, groupName, shareKey, Data) => {
  try {
    ...
    console.log("_SaveGroupDatabase3 ApiGroupRemove.js", erro)
  }
}
...
_SaveGroupDatabase2 = (tree, group, shareKey, Data) => {
  try {
    ...
    console.log("_SaveGroupDatabase2 ApiGroupRemove.js 2", error)
  }
}
...
_requestUpdate = (Secret, packet, group, path, lishcoPathNode) => {
  ...
  callApi(link.creategroup, 'POST', { data: data }).then(res => {
    if (res) {
      ...
      console.log("ApiGroupRemove.js network error");
    }
  })
}
...
_getprivateKeyInfo = (groupName) => {
  try {
    ...
    if (group[0]) {
      ...
      console.log("_rebuildSecret not found DirectPath info ApiGroupRemove.js", error)
    }
  } catch (error) {
    console.log("_rebuildSecret  DirectPath ApiGroupRemove.js", error)
  }
}
...
export default async function groupRemove(groupData, Check) {
